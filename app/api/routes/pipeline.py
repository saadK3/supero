...
@router.post("/runs", response_model=PipelineRunEnqueueResponse, status_code=status.HTTP_201_CREATED)
def create_run(
...
@router.post(
    "/orchestration/mock-run",
    response_model=OrchestrationRunResponse,
    status_code=status.HTTP_200_OK,
)
def run_orchestration(payload: OrchestrationStartRequest) -> OrchestrationRunResponse:
    return run_mock_orchestration(payload)
